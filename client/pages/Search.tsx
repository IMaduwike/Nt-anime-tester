  ...
  return (
    <div className="min-h-screen relative overflow-hidden" style={{ background: "var(--bg-main)", color: "var(--text-main)" }}>
      ...
      <div className="fixed inset-0 -z-40 overflow-hidden">
        <div className="absolute top-0 left-0 w-96 h-96 rounded-full filter blur-3xl animate-pulse" style={{ background: "rgba(158, 240, 255, 0.15)" }} />
        <div className="absolute top-1/3 right-0 w-80 h-80 rounded-full filter blur-3xl animate-pulse" style={{ background: "rgba(158, 240, 255, 0.08)", animationDelay: "1s" }} />
      </div>
      ...
      <section className="pt-32 pb-16 px-4 sm:px-6 lg:px-8 relative overflow-hidden">
        <div className="max-w-4xl mx-auto">
          <div className="text-center mb-8">
            <h1 className="font-bold mb-4" style={{ color: "var(--text-main)" }}>Search Anime</h1>
            <p className="text-lg" style={{ color: "var(--text-muted)" }}>Find your favorite anime series</p>
          </div>
          ...
          <form onSubmit={handleSearch} className="mb-8">
            <div className="glass relative rounded-xl p-2 flex items-center" style={{ borderColor: "var(--border-soft)" }}>
              ...
              {hasSearched && (
                <Button type="button" onClick={handleClearSearch} className="btn btn-secondary ml-2">
                  Clear
                </Button>
              )}
            </div>
            ...
            {loading ? (
              ...
              <>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                  {results.map((anime) => <AnimeCard key={anime.url} anime={anime} />)}
                </div>
              </>
            ) : (
      ...
      {!hasSearched && trending.length > 0 && (
        <section className="py-16 px-4 sm:px-6 lg:px-8 relative">
          <div className="max-w-7xl mx-auto">
            <h2 className="text-2xl font-bold mb-8" style={{ color: "var(--text-main)" }}>Trending Now</h2>
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
              {trending.map((anime) => <AnimeCard key={anime.url} anime={anime} />)}
            </div>
          </div>
        </section>
      )}
    ...
    <Link to={`/anime/${anime.url}`}>
      <div className="glass group relative rounded-xl overflow-hidden hover-lift h-full flex flex-col cursor-pointer" style={{ borderColor: "var(--border-soft)" }}>
        <div className="relative w-full h-48 overflow-hidden">
          ...
          />
          <div className="absolute inset-0" style={{ background: "linear-gradient(to bottom, transparent, var(--bg-main))" }} />
        </div>
        <div className="flex-1 p-4 flex flex-col justify-between">
          ...
          <Button className="w-full btn btn-primary text-xs" size="sm">Watch Now</Button>
        </div>
      </div>
    </Link>
  );
